...
export function AdminProvider({ children }) {
    ...
    async function loadAll() {
      ...
      try {
        ...
        const [uRes, aRes, pRes] = await Promise.all([
          ...
          apiFetch("/api/admin/agents", { signal: controller.signal }),
          apiFetch("/api/admin/properties", { signal: controller.signal }),
        ]);
  ...
  const totals = useMemo(() => {
    ...
    const forRent = properties.filter((p) =>
      /rent/i.test(p?.status || ""),
    ).length;
    const forSale = properties.filter((p) =>
      /sale/i.test(p?.status || ""),
    ).length;
  ...
  const togglePublished = async (propertyId, desired) => {
    ...
    setProperties((list) =>
      list.map((p) =>
        String(p.property_id) === String(propertyId)
          ? { ...p, isPublished: next }
          : p,
      ),
    );
